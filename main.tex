...
\quad \mathbf{m}_{\text{in}, i}^{\ell} &= \frac{1}{N_{\mathcal{N}_i}} \sum_{i \in \mathcal{N}_i}\mathbf{f}_{\text{out},i}^{\ell}
\end{align}
Here, $m_i$ is the atomic mass of atom $i$, and $\mathcal{C}_j \backslash j$ is the set of all neighborhood except the $j$-th one. Furthermore, the subgraph messages are also initialized to be of degree 0, 1, or 2. The model will encode subgraph-level scalar values such as energy using the type-0 features. On the other hand, the prediction of IR spectra necessitates a Fast Fourier Transform on the time derivatives of the dipole moment as
\begin{equation}
    I_{IR}(\omega) \propto \int \langle \mathbf{\dot{\mu}}(\tau), \mathbf{\dot{\mu}}(\tau + t) \rangle_\tau e^{-i \omega t}dt
\end{equation}
Where $\omega$ is vibrational frequency, $\tau$ is the time lag, $\mathbf{\dot{\mu}}$ is the time derivative of the dipole moment. Thus, these will be represented through type-1 features. For predicting Raman spectra, a useful property of the system to predict is the Placzek-type polarizability tensor $\alpha$ which is defined as $\mathbf{p} = \alpha \mathbf{E}$ where $\mathbf{p}$ is the dipole moment and $\mathbf{E}$ is the electric field. Using this, the Raman spectra can be calculated by
\begin{equation}
    I_\text{Raman}(\omega) \propto \frac{(\omega_\text{in} - \omega)^4}{\omega} \frac{1}{1 - \exp(-\frac{\hbar \omega}{k_B T})}S(a^2, \gamma^2)
\end{equation}
Here, $S(a^2, \gamma^2)$ is a combination of isotropic and anisotropic invariants of $\alpha$, $\omega$ is the vibrational frequency, $\omega_{in}$ is the frequency of the incident light, $k_B$ is the Boltzmann constant, and $T$ is the temperature. Therefore, $\ell=2$ for this property to propagate the correspondingly transformed object. 
\\\\ 
 Therefore, we define the inter-neighborhood graph edges as the distances connecting the center of masses of each neighborhood. The motivation behind this is to give stronger weight to the neighborhood messages that are closer to some node $i$ in subgraph $j$ since long-range chemical and electrical interactions fall off with distances. Furthermore, similar to the learnable weight matrices, $\mathbf{m_i}$ will be a $(2J + 1) \times (2J + 1)$ matrix that will be decomoposed into a the product of the corresponding Clebsch-Gordan coefficients times the direct sum of the orthonormal spherical harmonics functions evaluated at $\mathbf{x_i}$ for $\mathbf{m}_i$ to preserve equivariance once it acts on the feature vectors. Then, consider subgraph $i$, we update each node $i \in \mathcal{N}_i$ by the following:
\begin{equation}
...
\alpha_{ij}
&=
\frac{\exp(\mathbf{q}_i^T \mathbf{k}_{ij})}
     {\sum_{j \in \mathcal{N}_i \backslash i}
      \exp(\mathbf{q}_i^T \mathbf{k}_{ij})},
&
\mathbf{q}_i
&=
\bigoplus_{\ell \geq 0}
\sum_{k \geq 0}
\mathbf{W}_Q^{\ell k}
\mathbf{f}_{\mathrm{in},i}^{k},
&
\mathbf{k}_{ij}
&=
\bigoplus_{\ell \geq 0}
\sum_{k \geq 0}
\mathbf{W}_K^{\ell k}(\mathbf{x}_j-\mathbf{x}_i)
\mathbf{f}_{\mathrm{in},j}^{k},
\\[0.5em]
\beta_{ij}
&=
\frac{\exp(\mathbf{q}_{m,i}^T \mathbf{k}_{m,ij})}
     {\sum_{j \in \mathcal{C}_i \backslash i}
      \exp(\mathbf{q}_{m,i}^T \mathbf{k}_{m,ij})},
&
\mathbf{q}_{m,i}
&=
\bigoplus_{\ell \geq 0}
...
\sum_{k \geq 0}
\mathbf{W}_K^{\ell k}
(\mathbf{x}_{\mathrm{CM},j}-\mathbf{x}_{\mathrm{CM},i})
\mathbf{m}_{\mathrm{in},j}^{k},
\\[0.5em]
\gamma_{ij}
&=
\frac{\exp(\mathbf{q}_{i}^T \mathbf{k}_{m,ij})}
     {\sum_{j \in \mathcal{C}_i \backslash i}
      \exp(\mathbf{q}_{i}^T \mathbf{k}_{m,ij})},
&
\mathbf{q}_{i}
&=
\bigoplus_{\ell \geq 0}
